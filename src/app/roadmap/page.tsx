    ...
    <div className="min-h-screen bg-background pb-12 text-slate-900">
      ...
      <main className="container mx-auto px-4 pt-8 space-y-8">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <Button onClick={() => router.push('/onboarding')} className="rounded-full bg-accent hover:bg-accent/90 text-accent-foreground font-bold shadow-lg transition-transform hover:scale-105 active:scale-95">
        ...
        <section className="space-y-12 relative">
                  ...
                  <div className="flex flex-col md:flex-row justify-between gap-4">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="outline" className="bg-white text-orange-600 border-orange-200 uppercase font-bold text-[10px]">
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      ...
                      <Button 
                        ...
                        className="rounded-full bg-orange-600 hover:bg-orange-700 font-bold shadow-md"
                      >
          ...
          {roadmap.savingsPlans.map((plan, index) => (
            <div key={`${plan.id}-${index}`} className="relative flex gap-8 group">
              <div className="w-16 h-16 rounded-full bg-white border-4 border-primary flex items-center justify-center shadow-md shrink-0 z-10 transition-transform group-hover:scale-110">
                  ...
                  <div className="flex flex-col md:flex-row justify-between gap-4">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="secondary" className="uppercase text-[9px] font-bold">Fase 2: Ahorro</Badge>
                      </div>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        ...
                        <p className="font-bold text-lg">€{plan.goal.targetAmount}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] uppercase font-bold text-accent">Fondo al Finalizar</p>
                        <p className="font-bold text-lg text-accent">€{plan.totalEmergencySaved.toFixed(2)}</p>
                      </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden shadow-2xl rounded-2xl">
          <DialogHeader className="p-6 border-b shrink-0 bg-slate-50/50">
            <DialogTitle className="text-2xl font-headline font-bold">Editar Meta del Roadmap</DialogTitle>
            <DialogDescription>Ajusta los parámetros. Todo el Roadmap Maestro se recalculará automáticamente para mantener la coherencia temporal.</DialogDescription>
          </DialogHeader>
              ...
              <div className="p-8 space-y-12">
                <section className="space-y-6">
                  <h4 className="font-bold text-sm uppercase text-primary border-b pb-2 flex items-center gap-2">
                    <Target className="w-4 h-4" /> Datos Generales de la Meta
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                      <Label className="font-bold">Monto Objetivo (€)</Label>
                      <Input 
                        type="number"
                        placeholder="0.00"
                  ...
                  {(editingGoal.isExistingDebt || editingGoal.type === 'debt') && (
                    <div className="space-y-6 p-6 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
                      <p className="text-xs font-bold text-orange-600 uppercase flex items-center gap-1">
                        <TrendingDown className="w-3 h-3" /> Parámetros Bancarios para Fase 1
                      </p>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                          <Label className="text-xs">Cuota Mensual Obligatoria (€)</Label>
                          <Input 
                            type="number"
                        ...
                        <div className="space-y-2">
                          <Label className="text-xs">Comisión Amortización (%)</Label>
                          <Input 
                            type="number"
                            step="0.01"
          ...
          <DialogFooter className="p-6 border-t bg-slate-50/50 shrink-0">
            <Button variant="ghost" onClick={() => setIsEditDialogOpen(false)} className="rounded-full">Cancelar</Button>
            <Button onClick={handleUpdateGoal} className="rounded-full shadow-lg bg-primary hover:bg-primary/90 font-bold px-8">
      ...
      <Dialog open={isPortfolioDialogOpen} onOpenChange={setIsPortfolioDialogOpen}>
        <DialogContent className="max-w-6xl h-[90vh] flex flex-col p-0 overflow-hidden shadow-2xl rounded-2xl">
          <DialogHeader className="p-6 border-b shrink-0 bg-orange-50/30">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
              <div>
                <DialogTitle className="text-2xl font-headline font-bold text-orange-950">Análisis Fase 1: Portafolio de Deudas</DialogTitle>
                <DialogDescription className="text-orange-900/60">Simulación simultánea con aceleración por liberación de cuotas.</DialogDescription>
              </div>
              <Badge className="bg-orange-600 text-white font-bold px-4 py-1.5 rounded-full shadow-sm w-fit uppercase text-[10px]">
                Prioridad: {roadmap.debtPrioritization.toUpperCase()}
              </Badge>
            </div>
          </DialogHeader>
            ...
            <ScrollArea className="flex-1">
              <div className="p-8 space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <Card className="border-none shadow-sm bg-orange-100/30">
                    <CardHeader className="py-3 px-4">
                      <CardDescription className="text-[10px] uppercase font-bold text-orange-700">Meses Duración</CardDescription>
                      <CardTitle className="text-lg">{viewingPortfolio.totalMonths}</CardTitle>
                    </CardHeader>
                  </Card>
                  <Card className="border-none shadow-sm bg-red-100/20">
                    <CardHeader className="py-3 px-4">
                  ...
                  <Card className="border-none shadow-sm bg-green-100/20">
                    <CardHeader className="py-3 px-4">
                      <CardDescription className="text-[10px] uppercase font-bold text-green-700">Extra Aplicado</CardDescription>
                  ...
                  <Card className="border-none shadow-sm bg-slate-100">
                    <CardHeader className="py-3 px-4">
                      <CardDescription className="text-[10px] uppercase font-bold text-slate-700">Deudas Finalizadas</CardDescription>
                  ...
                  <Table className="w-full">
                    <TableHeader className="bg-slate-50 sticky top-0 z-10 border-b">
                      <TableRow className="hover:bg-transparent">
                        <TableHead className="w-24 text-center font-bold whitespace-nowrap">Mes</TableHead>
                        <TableHead className="text-center font-bold text-red-600 whitespace-nowrap">Intereses</TableHead>
                        <TableHead className="text-center font-bold text-primary whitespace-nowrap">Aporte Extra</TableHead>
                        <TableHead className="text-center font-bold whitespace-nowrap">Total Pagado</TableHead>
                        <TableHead className="text-center font-bold text-orange-600 whitespace-nowrap">Deuda Restante</TableHead>
                        <TableHead className="text-right font-bold text-accent whitespace-nowrap">Fondo Acum.</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {viewingPortfolio.timeline.map((row) => (
                        <TableRow key={row.month} className="hover:bg-slate-50 transition-colors">
                          ...
                          <TableCell className="text-center text-red-500 font-mono text-[11px] whitespace-nowrap">€{row.totalInterestPaid}</TableCell>
                          <TableCell className="text-center text-primary font-bold font-mono text-[11px] whitespace-nowrap">€{row.totalExtraPaid}</TableCell>
                          <TableCell className="text-center font-mono text-[11px] whitespace-nowrap">€{row.totalPaid}</TableCell>
                          <TableCell className="text-center text-orange-600 font-bold font-mono text-[11px] whitespace-nowrap">€{row.remainingTotalDebt}</TableCell>
                          <TableCell className="text-right text-accent font-bold font-mono text-[11px] whitespace-nowrap">€{row.cumulativeEmergencyFund}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
          ...
          <DialogFooter className="p-4 border-t bg-slate-50 shrink-0">
            <Button onClick={() => setIsPortfolioDialogOpen(false)} className="rounded-full px-8 font-bold">Cerrar Análisis de Cartera</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent className="max-w-6xl h-[90vh] flex flex-col p-0 overflow-hidden shadow-2xl rounded-2xl">
          <DialogHeader className="p-6 border-b shrink-0 bg-blue-50/20">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
              <div>
                <DialogTitle className="text-2xl font-headline font-bold text-slate-900">Análisis Fase 2: {viewingPlan?.goal.name}</DialogTitle>
                <DialogDescription className="text-slate-500">Plan secuencial de ahorro optimizado con rentabilidad de fondo.</DialogDescription>
              </div>
              <Badge className="bg-primary text-white font-bold px-4 py-1.5 rounded-full shadow-sm w-fit uppercase text-[10px]">Ahorro Activo</Badge>
            </div>
          </DialogHeader>
              ...
              <div className="p-8 space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <Card className="border-none shadow-sm bg-slate-50">
                    <CardHeader className="py-3 px-4">
                      <CardDescription className="text-[10px] uppercase font-bold text-slate-600">Monto Objetivo</CardDescription>
                  ...
                  <Card className="border-none shadow-sm bg-blue-50">
                    <CardHeader className="py-3 px-4">
                      ...
                      <CardTitle className="text-lg">€{viewingPlan.monthlyContributionExtra}/m</CardTitle>
                    </CardHeader>
                  </Card>
                  ...
                  <Card className="border-none shadow-sm bg-orange-50">
                    <CardHeader className="py-3 px-4">
                      <CardDescription className="text-[10px] uppercase font-bold text-orange-700">Fecha Estimada</CardDescription>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-6">
                    <h4 className="font-headline font-bold flex items-center text-sm text-slate-800">
                      <Clock className="w-4 h-4 mr-2 text-primary" /> Evolución Mensual del Ahorro
                    </h4>
                    <div className="rounded-2xl border bg-white shadow-md overflow-hidden">
                      <Table className="w-full">
                        <TableHeader className="bg-slate-50 sticky top-0 z-10 border-b">
                          <TableRow className="hover:bg-transparent text-[10px]">
                            <TableHead className="w-24 text-center border-r font-bold whitespace-nowrap">Mes</TableHead>
                            <TableHead className="text-center font-bold text-primary whitespace-nowrap">Aporte Ahorro</TableHead>
                            <TableHead className="text-center font-bold text-accent whitespace-nowrap">Aporte Fondo</TableHead>
                            <TableHead className="text-center font-bold text-green-600 whitespace-nowrap">Interés Ganado</TableHead>
                            <TableHead className="text-right font-bold whitespace-nowrap">Fondo Acumulado</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {viewingPlan.monthlyTable.map((row) => (
                            <TableRow key={row.month} className="hover:bg-slate-50 transition-colors">
                              ...
                              <TableCell className="text-center text-primary font-bold font-mono text-[11px] whitespace-nowrap">€{row.extraPrincipalPaid.toFixed(2)}</TableCell>
                              <TableCell className="text-center text-accent font-bold font-mono text-[11px] whitespace-nowrap">€{row.emergencyFundContribution.toFixed(2)}</TableCell>
                              <TableCell className="text-center text-green-600 font-mono text-[11px] whitespace-nowrap">€{row.savingsInterestEarned.toFixed(2)}</TableCell>
                              <TableCell className="text-right font-bold font-mono text-[11px] whitespace-nowrap">€{row.cumulativeEmergencyFund.toFixed(2)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                  ...
                  <div className="space-y-6">
                    <section className="space-y-4">
                      <h4 className="font-headline font-bold flex items-center text-sm"><Info className="w-4 h-4 mr-2 text-primary" /> Contexto del Plan</h4>
                      <div className="p-5 bg-blue-50/50 rounded-2xl border border-dashed border-blue-200 text-[11px] space-y-4 leading-relaxed text-slate-700 shadow-sm">
                        <p><strong>Estrategia Global:</strong> {viewingPlan.strategy === 'emergency_first' ? 'Seguridad Máxima' : viewingPlan.strategy === 'balanced' ? 'Equilibrio' : 'Aceleración Meta'}.</p>
                        <p><strong>Fondo de Emergencia:</strong> Tu objetivo de seguridad de €{viewingPlan.targetEmergencyFund} se recalcula con la rentabilidad TAE cada mes.</p>
                        <p className="flex items-start gap-2 text-slate-500">
                          <CheckCircle2 className="w-3 h-3 text-green-500 shrink-0 mt-0.5" /> 
                          Este plan hereda el fondo acumulado de la Fase 1 de deudas, optimizando el punto de partida.
                        </p>
                      </div>
                    </section>
                  </div>
          ...
          <DialogFooter className="p-4 border-t bg-slate-50 shrink-0">
            <Button onClick={() => setIsViewDialogOpen(false)} className="rounded-full px-8 font-bold">Cerrar Análisis de Ahorro</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
